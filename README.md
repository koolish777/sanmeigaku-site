...
算命学を中心に九星気学・ジョーティッシュ（インド式占星術）を重ねた総合運勢診断サイトのプロトタイプです。

## 現行バージョン

- **現行作業ファイル：v51**（ローカル作業状態 / 2026-06-14）
- 公開用作業ファイル：このフォルダの `index.html`（title / badge / footer はv51）。2026-06-13に自然法人体星図B差戻しを修正し、v51表示配置もadmin基準へ同期。seed 2026061302で朱学院5件/自然法5件とも公式照合PASS。2026-06-14に顧客向け・商品向けの西洋占星術表現を外し、ジョーティッシュへ統一。sha256 `E9B9A16595825CB78081D723F1FF00D25BE3056624748C1C706361CB6E925F2D`。
...
- **無料＝入口型**：命式の図（陰占・陽占）＋ジョーティッシュのかんたんな読み解き＋今日の運勢（ラッキーアイテム/カラー）＋詩の断片。「見てよかった」で完結する軽い入口。
- **②先生の鑑定**：算命学・気学による具体的な読み解き→総合アドバイスで結論。
- **③個別・最高級**：ジョーティッシュと四柱推命まで重ねた総合判断（上位商品として扱う）。
